...
const SignUp: React.FC = () => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Registra al usuario
      const queryString = Object.keys(formData).map(key => `${encodeURIComponent(key)}=${encodeURIComponent(formData[key as keyof typeof formData])}`).join('&');
      const response2 = await axios.post(`http://localhost:1111/barber_shop_booking_hub/cuenta/registrar?${queryString}`, null, {
        headers: {
      ...
      const queryString2 = Object.keys(formData).map(key => `${encodeURIComponent(key)}=${encodeURIComponent(formData[key as keyof typeof formData])}`).join('&');
      const loginResponse = await axios.post(`http://localhost:1111/barber_shop_booking_hub/cuenta/login?${queryString2}`);
